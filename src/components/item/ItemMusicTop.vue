<template>
  ...
  <div class="itemMusicTop">
    ...
    <div class="itemLeft">
      <!-- 左箭头定义一个点击事件@click $router.go(-1)返回上一页 还有$router.back-->
...
export default {
  setup(props) {
    console.log(props);
    // 通过props进行传值 判断如果数据拿不到 就从sessionStorage中获取
  ...
  //props接收父组件传来的参数 playList 这里props必须双引号吗？？
  props: ["playlist"],
};
</script>
...
.itemMusicTop {
  ...
  .bgimg {
    width: 100%;
    height: 11rem;
    position: fixed; //浮动 父元素要设置relative
    z-index: -1; //这个应该是图层
    filter: blur(30px); //设置背景虚化
  }
}
.itemTopContent {
  width: 100%;
